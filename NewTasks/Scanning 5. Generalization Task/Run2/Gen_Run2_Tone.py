...
expName = 'Gen_Run2_Tone'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
    'run': '002',
}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/stashjian/Documents/Melbourne/Adolescent_Safety/GeneralizationTask_windows/Run2/Gen_Run2_Tone.py',
...
Block_A_Confidence_Resp = keyboard.Keyboard()
...
Block_L_Confidence = visual.ImageStim(
    ...
    flipHoriz=False, flipVert=False,
    texRes=128.0, interpolate=True, depth=-2.0)
Block_L_Resp = keyboard.Keyboard()
...
for thisFullLoop in FullLoop:
    ...
    for thisLoopA in loopA:
        ...
        # continueRoutine = True
        # # update component parameters for each repeat
        # Block_A_Confidence_Resp.keys = []
        # Block_A_Confidence_Resp.rt = []
        # _Block_A_Confidence_Resp_allKeys = []
        # # keep track of which components have finished
        # Block_AComponents = [Block_A_ITI, Block_A_Context, Block_A_Confidence, Block_A_Confidence_Resp]
        ...
        #     if Block_A_Confidence.status == STARTED:
        #         # is it time to stop? (based on global clock, using actual start)
        #         if tThisFlipGlobal > Block_A_Confidence.tStartRefresh + 7-frameTolerance:
        #             # keep track of stop time/frame for later
        ...
        #     # *Block_A_Confidence_Resp* updates
        #     waitOnFlip = False
            
        #     # if Block_A_Confidence_Resp is starting this frame...
        #     if Block_A_Confidence_Resp.status == NOT_STARTED and tThisFlip >= 5.5-frameTolerance:
        #         # keep track of start time/frame for later
        #         Block_A_Confidence_Resp.frameNStart = frameN  # exact frame index
        #         Block_A_Confidence_Resp.tStart = t  # local t and not account for scr refresh
        #         Block_A_Confidence_Resp.tStartRefresh = tThisFlipGlobal  # on global time
        #         win.timeOnFlip(Block_A_Confidence_Resp, 'tStartRefresh')  # time at next scr refresh
        #         # add timestamp to datafile
        #         thisExp.timestampOnFlip(win, 'Block_A_Confidence_Resp.started')
        #         # update status
        #         Block_A_Confidence_Resp.status = STARTED
        #         # keyboard checking is just starting
        #         waitOnFlip = True
        #         win.callOnFlip(Block_A_Confidence_Resp.clock.reset)  # t=0 on next screen flip
        #         win.callOnFlip(Block_A_Confidence_Resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
        #     # if Block_A_Confidence_Resp is stopping this frame...
        #     if Block_A_Confidence_Resp.status == STARTED:
        #         # is it time to stop? (based on global clock, using actual start)
        #         if tThisFlipGlobal > Block_A_Confidence_Resp.tStartRefresh + 7-frameTolerance:
        #             # keep track of stop time/frame for later
        #             Block_A_Confidence_Resp.tStop = t  # not accounting for scr refresh
        #             Block_A_Confidence_Resp.frameNStop = frameN  # exact frame index
        #             # add timestamp to datafile
        #             thisExp.timestampOnFlip(win, 'Block_A_Confidence_Resp.stopped')
        #             # update status
        #             Block_A_Confidence_Resp.status = FINISHED
        #             Block_A_Confidence_Resp.status = FINISHED
        #     if Block_A_Confidence_Resp.status == STARTED and not waitOnFlip:
        #         theseKeys = Block_A_Confidence_Resp.getKeys(keyList=['a','b','c','d', 'A', 'B', 'C', 'D'], waitRelease=False)
        #         _Block_A_Confidence_Resp_allKeys.extend(theseKeys)
        #         if len(_Block_A_Confidence_Resp_allKeys):
        #             Block_A_Confidence_Resp.keys = [key.name for key in _Block_A_Confidence_Resp_allKeys]  # storing all keys
        #             Block_A_Confidence_Resp.rt = [key.rt for key in _Block_A_Confidence_Resp_allKeys]
        #             # a response ends the routine
        #             continueRoutine = False
        ...
        #         thisComponent.setAutoDraw(False)
        # # check responses
        # if Block_A_Confidence_Resp.keys in ['', [], None]:  # No response was made
        #     Block_A_Confidence_Resp.keys = None
        # loopA.addData('Block_A_Confidence_Resp.keys',Block_A_Confidence_Resp.keys)
        # if Block_A_Confidence_Resp.keys != None:  # we had a response
        #     loopA.addData('Block_A_Confidence_Resp.rt', Block_A_Confidence_Resp.rt)
        # # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        # if routineForceEnded:
        #     routineTimer.reset()
        # else:
        ...
        loop1 = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('Battles_Run2_A.xlsx'),
        ...
        for thisLoop1 in loop1:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            firststim_img_loop1.setImage(FirstStimA)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_stim1_loop1.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_stim1_loop1.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim1_loop1.setImage(ConfidenceA)
            ...
            jitter_isi_1 = rand.choices([0.5, 0.5, 0.5, 1.0, 1.0, 1.5, 2.0], k=54) # create the list
            ...
            continueRoutine = True
            # update component parameters for each repeat
            secondstim_img_loop1.setImage(SecondStimA)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_stim2_loop1.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_stim2_loop1.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim2_loop1.setImage(ConfidenceA)
            ...
            if routineForceEnded:
                ...
                routineTimer.addTime(-0.2500000)
            
            # --- Prepare to start Routine "outcome_loop1" ---
            continueRoutine = True
            # update component parameters for each repeat
            outcome_img_loop1.setImage(OutcomeA)
            Tone_loop1.setSound(ToneA, secs=1.3, hamming=True)
    ...
    for thisLoopH in loopH:
        ...
        loop2 = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('Battles_Run2_H.xlsx'),
        ...
        for thisLoop2 in loop2:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            firststim_img_loop2.setImage(FirstStimH)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_stim1_loop2.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_stim1_loop2.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim1_loop2.setImage(ConfidenceH)
            ...
            jitter_isi_2 = rand.choices([0.5, 0.5, 0.5, 1.0, 1.0, 1.5, 2.0], k=54) # create the list
            ...
            continueRoutine = True
            # update component parameters for each repeat
            secondstim_img_loop2.setImage(SecondStimH)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_loop2.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_loop2.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim2_loop2.setImage(ConfidenceH)
            ...
            continueRoutine = True
            # update component parameters for each repeat
            outcome_image_loop2.setImage(OutcomeH)
            Tone_loop2.setSound(ToneH, secs=1.3, hamming=True)
    ...
    for thisLoopL in loopL:
        ...
        # Block_L_Resp.keys = []
        # Block_L_Resp.rt = []
        # _Block_L_Resp_allKeys = []
        # # keep track of which components have finished
        # Block_LComponents = [Block_L_ITI, Block_L_Context, Block_L_Confidence, Block_L_Resp]
        ...
        #     if Block_L_Confidence.status == STARTED:
        #         # is it time to stop? (based on global clock, using actual start)
        #         if tThisFlipGlobal > Block_L_Confidence.tStartRefresh + 5-frameTolerance:
        #             # keep track of stop time/frame for later
        ...
        #     # *Block_L_Resp* updates
        #     waitOnFlip = False
            
        #     # if Block_L_Resp is starting this frame...
        #     if Block_L_Resp.status == NOT_STARTED and tThisFlip >= 5.5-frameTolerance:
        #         # keep track of start time/frame for later
        #         Block_L_Resp.frameNStart = frameN  # exact frame index
        #         Block_L_Resp.tStart = t  # local t and not account for scr refresh
        #         Block_L_Resp.tStartRefresh = tThisFlipGlobal  # on global time
        #         win.timeOnFlip(Block_L_Resp, 'tStartRefresh')  # time at next scr refresh
        #         # add timestamp to datafile
        #         thisExp.timestampOnFlip(win, 'Block_L_Resp.started')
        #         # update status
        #         Block_L_Resp.status = STARTED
        #         # keyboard checking is just starting
        #         waitOnFlip = True
        #         win.callOnFlip(Block_L_Resp.clock.reset)  # t=0 on next screen flip
        #         win.callOnFlip(Block_L_Resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            
        #     # if Block_L_Resp is stopping this frame...
        #     if Block_L_Resp.status == STARTED:
        #         # is it time to stop? (based on global clock, using actual start)
        #         if tThisFlipGlobal > Block_L_Resp.tStartRefresh + 5-frameTolerance:
        #             # keep track of stop time/frame for later
        #             Block_L_Resp.tStop = t  # not accounting for scr refresh
        #             Block_L_Resp.frameNStop = frameN  # exact frame index
        #             # add timestamp to datafile
        #             thisExp.timestampOnFlip(win, 'Block_L_Resp.stopped')
        #             # update status
        #             Block_L_Resp.status = FINISHED
        #             Block_L_Resp.status = FINISHED
        #     if Block_L_Resp.status == STARTED and not waitOnFlip:
        #         theseKeys = Block_L_Resp.getKeys(keyList=['a','b','c','d', 'A', 'B', 'C', 'D'], waitRelease=False)
        #         _Block_L_Resp_allKeys.extend(theseKeys)
        #         if len(_Block_L_Resp_allKeys):
        #             Block_L_Resp.keys = [key.name for key in _Block_L_Resp_allKeys]  # storing all keys
        #             Block_L_Resp.rt = [key.rt for key in _Block_L_Resp_allKeys]
        #             # a response ends the routine
        #             continueRoutine = False
        ...
        #         thisComponent.setAutoDraw(False)
        # # check responses
        # if Block_L_Resp.keys in ['', [], None]:  # No response was made
        #     Block_L_Resp.keys = None
        # loopL.addData('Block_L_Resp.keys',Block_L_Resp.keys)
        # if Block_L_Resp.keys != None:  # we had a response
        #     loopL.addData('Block_L_Resp.rt', Block_L_Resp.rt)
        # # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        # if routineForceEnded:
        #     routineTimer.reset()
        # else:
        ...
        loop3 = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('Battles_Run2_L.xlsx'),
        ...
        for thisLoop3 in loop3:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            firststim_img_loop3.setImage(FirstStimL)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_stim1_loop3.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_stim1_loop3.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim1_loop3.setImage(ConfidenceL)
            ...
            jitter_isi_3 = rand.choices([0.5, 0.5, 0.5, 1.0, 1.0, 1.5, 2.0], k=54) # create the list
            ...
            continueRoutine = True
            # update component parameters for each repeat
            secondstim_img_loop3.setImage(SecondStimL)
            # keep track of which components have finished
            ...
            while continueRoutine and routineTimer.getTime() < 0.25:
                ...
                if isi_conf_stim2_loop3.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > isi_conf_stim2_loop3.tStartRefresh + .3-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            continueRoutine = True
            # update component parameters for each repeat
            conf_img_stim2_loop3.setImage(ConfidenceL)
            ...
            continueRoutine = True
            # update component parameters for each repeat
            outcome_img_loop3.setImage(OutcomeL)
            Tone_loop3.setSound(ToneL, secs=1.3, hamming=True)
